    ...
    with open(csv_filename, mode='r', newline='') as csvfile:
        ...
        for row in reader:
            ...
            if len(final_state_amd64) == 1 and len(final_state_aarch64) ==1 :
                ...
                # 输出寄存器及其值，具体实现依赖于execute_assembly函数的定义
                # print("AMD64 registers: ", amd64_registers_value)
                # print("AARCH64 registers: ", aarch64_registers_value)
                ...
                for reg in registers_output:
                    ...
                    if reg in mappings4:
                        ...
                        if s.satisfiable(extra_constraints=[arm_expr != amd_expr]):
                            flag = False
                            # print("存在值使得等式不成立,因此两段汇编代码在给定的逻辑下不等价。")

                            # print("不存在值使得等式不成立,因此两段汇编代码在给定的逻辑下等价。")
                if flag :
                    print("两段汇编只有一条路径，且两段汇编等价")
                else :
                    print("两段汇编不等价")
            elif len(final_state_amd64) > 1 and len(final_state_aarch64) > 1 and len(final_state_amd64) == len(final_state_amd64):

                for i in range(len(final_state_amd64)):
                    print("路径次数",i)
                    final_state_amd64_multi = final_state_amd64[i]
                    final_state_aarch64_multi = final_state_aarch64[i]
                    amd64_registers_value = get_register_values(final_state_amd64_multi, amd64_registers)
                    aarch64_registers_value = get_register_values(final_state_aarch64_multi, aarch64_registers)
                    registers_output = find_changed_registers(amd64,amd64_registers_value,final_state_amd64_multi)
                    arm_registers_output = find_changed_registers(aarch64,aarch64_registers_value,final_state_aarch64_multi)
                    print("amd_registers_output输出寄存器有: ",registers_output)
                    print("arm_registers_output输出寄存器有: ",arm_registers_output)
                    mappings = {
                                'rax': 'x4', 'rcx': 'x5', 'rdx': 'x6', 'rbx': 'x7',
                                'rsp': 'x8', 'rbp': 'x9', 'rsi': 'x10', 'rdi': 'x11',
                                'r8': 'x12', 'r9': 'x13', 'r10': 'x14', 'r11': 'x15',
                    ...
                    for mem_amd_obj in mem_amd:
                        for mem_arm_obj in mem_arm:
                            if mem_amd_obj.disp == mem_arm_obj.disp and mem_amd_obj.reg in mappings and mem_arm_obj.reg == mappings[mem_amd_obj.reg]:
                                memory_content_amd1 = mem_amd_obj.value
                                memory_content_arm1 = mem_arm_obj.value
                                memory_content_amd2 = final_state_amd64_multi.memory.load(mem_amd_obj.address, 8, endness=final_state_amd64_multi.arch.memory_endness)
                                memory_content_arm2 = final_state_aarch64_multi.memory.load(mem_arm_obj.address, 8, endness=final_state_aarch64_multi.arch.memory_endness)
                    ...
                    for reg in amd64_registers:
                        notmemreg =True
                        for mem_amd_obj in mem_amd:
                            if mem_amd_obj.reg == reg:
                                notmemreg =False

                        if reg in mappings2 and notmemreg:
                            arm_reg = mappings2.get(reg)
                            arm_expr = aarch64_registers_value[arm_reg]
                            amd_expr = amd64_registers_value[reg]

                            if final_state_amd64_multi.solver.max(amd_expr)!= final_state_aarch64_multi.solver.max(arm_expr) or final_state_amd64_multi.solver.min(amd_expr)!= final_state_aarch64_multi.solver.min(arm_expr):
                                flag = False
                                print("跳转条件不相符")
                        if reg in mappings3 and notmemreg:
                            arm_reg = mappings3.get(reg)        
                            arm_expr = claripy.Extract(15, 0, aarch64_registers_value[arm_reg])
                            amd_expr = amd64_registers_value[reg]
                            ...
                            if final_state_amd64_multi.solver.max(amd_expr)!= final_state_aarch64_multi.solver.max(arm_expr) or final_state_amd64_multi.solver.min(amd_expr)!= final_state_aarch64_multi.solver.min(arm_expr):
                                flag = False
                                print("跳转条件不相符")
                        if reg in mappings4 and notmemreg:
                            arm_reg = mappings4.get(reg)
                            arm_expr = claripy.Extract(7, 0, aarch64_registers_value[arm_reg])
                            amd_expr = amd64_registers_value[reg]
                ...
                if flag :
                    print("两段汇编具有多条路径，且两段汇编等价")
                else :
                    print("两段汇编不等价")
                    
            else :
                print(row)
                print('由于路径数目不同不满足等价条件')
if __name__ == "__main__":
    csv_filename = '/home/kingdom/PalmTree-master/angr/code.csv'
    main(csv_filename)
